			...
			for (var p in preset._items) {
				...
				if (preset._items[p].slotId === "mod_magazine") {
					...
					if (item._props.Chambers.length > 0) {
						cartridges._tpl = utility.getArrayValue(item._props.Chambers[0]._props.filters[0].Filter);
					}
...
class LocationServer {
	...
	generate(name) {
		...
		/* This code doesn't work and I can't be bothered fixing it - Nevermind
