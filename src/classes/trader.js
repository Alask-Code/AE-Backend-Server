...
class TraderServer {
    ...
    getCustomization(traderID, sessionID) {
        ...
        if (typeof global._database.suits == 'undefined') {
            global._database.suits = {};
        }

        if (typeof global._database.suits[traderID] == 'undefined') {
            global._database.suits[traderID] = fileIO.readParsed('user/cache/customization_' + traderID + '.json');
        }

        const suitArray = global._database.suits[traderID];
...
function generateFenceAssort() {
    // Lifetime in seconds
    const fence_assort_lifetime = 600;

    // Current time in seconds
    const current_time = Math.floor(new Date().getTime() / 1000);

    if (typeof global._database.fenceLastAssort == 'undefined' || (global._database.fenceLastAssort + fence_assort_lifetime) < current_time) {
        ...
        for (let i = 0; i < FENCE_ASSORT_SIZE; i++) {
            ...
            let random_item_children = trader_f.handler.iter_item_children_recursively(random_item, fence_base_assort)

            trader_f.handler.generate_item_ids(random_item, ...random_item_children);
        ...
        global._database.fenceLastAssort = current_time; // Time in seconds
    }
}
